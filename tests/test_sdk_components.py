...
def test_imports():
    from offers_sdk import (
        OffersClient, OffersClientSync, OffersAPISettings, AuthManager,
        AuthError, OffersAPIError, Middleware, PluginManager,
        RequestPlugin, ResponsePlugin, TokenStore, FileTokenStore
    )
    from offers_sdk.transport import get_transport
    from offers_sdk.plugins.examples import (
        LoggingPlugin, AuthenticationPlugin, MetricsPlugin
    )
    assert True
...
async def test_plugin_manager():
    ...
    await manager.process_request("GET", "https://test.com", {"x": "y"}, None, None, None)
...
async def test_middleware():
    ...
    class DummyMiddleware:
        async def on_request(self, *a, **kw): return None
        async def on_response(self, r): return r
